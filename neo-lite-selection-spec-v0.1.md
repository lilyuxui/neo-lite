...
When `error` is true and the Selection trigger receives focus, use the Neo-Lite destructive focus treatment:

```text
label: error
value: foreground
border: destructive-border
focus ring: 0 0 0 2px var(--destructive-focus)
```
...
or the existing Tailwind semantic utility if already bridged, conceptually:

```text
shadow-destructive-focus
```

Do not use the standard purple `--focus-ring` while the Selection is in error state.

Do not hard-code the destructive-focus colour in `Selection.tsx`.

Do not clip the outer error focus ring.

The error-focus visual state must come from the real combination of:
...
Do not expose `state="Error-focus"` publicly.
...
22. Focused error Selection uses `--destructive-focus-shadow`, not the standard purple focus ring.
23. Error-focus keeps the destructive border while focused.
24. Disabled field uses 30% opacity.
...
disabled options
error + error-focus visual states
disabled
controlled value
uncontrolled value
